...
public class ShelfScreen extends AbstractContainerScreen<ShelfMenu>
{
    private final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(AestheticShelving.MOD_ID,
            "textures/gui/" + getWoodName(ModBlocks.SHELVES, Objects.requireNonNull(menu.getSlot(0).container
                    instanceof ShelfBlockEntity be ? be : null)) + ".png");
    ...
    {
        ShelfBlockEntity blockEntity = Objects.requireNonNull(menu.getSlot(0).container
                instanceof ShelfBlockEntity be ? be : null);

        graphics.drawString(this.font, this.title, this.titleLabelX, this.titleLabelY,
                WOOD_COLORS.get(getWoodName(ModBlocks.SHELVES, blockEntity)), false);
